...
=============================================================
   ...
   VERSIÓN CORREGIDA - Todas las vulnerabilidades solucionadas
  
  Correcciones implementadas:
    V-01   Consultas parametrizadas en login
    V-02   Consultas parametrizadas en búsqueda
    V-03   Contraseñas hasheadas con werkzeug.security
    V-04   SECRET_KEY desde variable de entorno
    V-05   debug=False en producción
    V-06   Consulta SQL NO se expone en interfaz
    V-07  Menú Admin solo visible para admins
...
app.config["SECRET_KEY"] = os.environ.get(
    "SECRET_KEY",
    "dev-secret-key-solo-para-desarrollo-local"
)
...
def init_db():
    ...
    #  V-03: Contraseñas hasheadas con pbkdf2
    cur.execute("SELECT COUNT(*) FROM users")
    if cur.fetchone()[0] == 0:
        users = [
            ("admin",   generate_password_hash("Admin123", method="pbkdf2"),   "admin"),
            ("analyst", generate_password_hash("Analyst123", method="pbkdf2"), "user"),
            ("student", generate_password_hash("Student123", method="pbkdf2"), "user"),
        ]
        cur.executemany(
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            flash("Error en la base de datos. Contacte al administrador.", "error")
            conn.close()
            return render_template("login.html")
        conn.close()
 
        #  V-03: Verificación con check_password_hash
...
def search():
    ...
    if request.method == "POST":
        ...
        try:
            search_term = f"%{term}%"
            books = conn.execute(query, (search_term, search_term, search_term)).fetchall()
        except Exception as e:
            flash("Error en la búsqueda. Intente de nuevo.", "error")
        conn.close()
 
    #  V-06: NO se pasa raw_query
...
#  V-05: debug depende de variable de entorno
